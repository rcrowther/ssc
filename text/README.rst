...
Libraries
---------
One bad start is lost dependancies for the code to be documented. ``ssc`` needs to know about libraries the code uses, as it does a kind of compile to make the documentation. By default, ``ssc`` looks in `lib/` then `Lib/`, so you may be fine. If not, make a `build.ssc` file (see below) to override, e.g. with this line, ::
...
Commandline
-------------
A useful sometimes extra commandline option, ::
...
I havn't decided which options to keep yet. Best say, "subject to alteration".
...
If ``ssc`` is run in a folder with a `build.ssc` file, it reads the file and adds any configuaration it finds there to the default.
